...
def train_seq2seq_with_visualization(net, data_iter, lr, num_epochs, tgt_vocab, device):
    """训练序列到序列模型并可视化损失"""
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_iter:
            ...
            Y_hat = net(X, dec_input, X_valid_len)  # 只返回一个 tensor
            ...
            with torch.no_grad():
                ...
                losses.append(metric[0] / metric[1])  # 记录损失
    print(f'loss {metric[0] / metric[1]:.3f}, {metric[1] / timer.stop():.1f} '
          f'tokens/sec on {str(device)}')

# 训练模型并可视化损失
train_seq2seq_with_visualization(net, train_iter, lr, num_epochs, tgt_vocab, device)
